...
// ---- QR parsing (length/spec/railType) ----
...
export default function App() {
  ...
  const loadedAt = "WalvisBay"; // static

  const [pending, setPending] = useState(null);
  const [qrExtras, setQrExtras] = useState({
    grade: "",
    railType: "",
    spec: "",
    lengthM: "",
  ...
  // force remount of Scanner when starting the next scan (clean camera init)
  ...
  const findDuplicates = (serial) => {
    ...
    if (!key) return [];
    return scans.filter(
      (r) => String(r.serial || "").trim().toUpperCase() === key
    );
  };
  ...
  const onDetected = (rawText) => {
    ...
    if (serial && findDuplicates(serial).length > 0) {
      warnBeep();
      setDupPrompt({
        serial: String(serial).toUpperCase(),
        matches: findDuplicates(serial),
        candidate: {
          pending: {
            serial: String(serial).toUpperCase(),
  ...
  const confirmPending = async () => {
    ...
    const rec = {
      ...
      wagon3Id,
      receivedAt,
      loadedAt: "WalvisBay", // static on save
      timestamp: new Date().toISOString(),
      grade: qrExtras.grade,
      railType: qrExtras.railType,
      spec: qrExtras.spec,
      lengthM: qrExtras.lengthM,
      qrRaw: pending.raw || pending.serial, // saved for QR image export route (xlsx)
    };
    ...
    try {
      ...
      // ready for next item, but keep fields if you prefer
      setScannerKey((k) => k + 1);
    } catch (e) {
    ...
    <div className="container" style={{ paddingTop: 20, paddingBottom: 20 }}>
      <header className="app-header">
        ...
        >
          ...
          <button className="btn btn-outline" onClick={goBackToStart}>
            Back to Start
          </button>
        </div>
      </header>
      ...
      <div className="grid" style={{ marginTop: 20 }}>
        ...
        <section className="card">
          ...
          {pending && (
            <div className="notice" style={{ marginTop: 10 }}>
              ...
              <div className="meta">
                Captured at: {new Date(pending.capturedAt).toLocaleString()}
              </div>
            </div>
          )}
        </section>
        ...
        <section className="card">
          ...
          >
            ...
            <button className="btn btn-outline" onClick={resetForNextScan}>
              Scan Next
            </button>
            <button className="btn btn-outline" onClick={goBackToStart}>
              Back to Start
