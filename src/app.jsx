...
function parseQrPayload(raw) {
  ...
  if (!serial) {
    const cand = tokens.filter(x => /^[A-Z0-9-]{8,32}$/i.test(x))
...
export default function App() {
  ...
  useEffect(() => {
    fetch(api('/staged')).then(r => r.ok && r.json().then(setScans)).catch(()=>{});
  ...
  const confirmPending = async () => {
    ...
    try {
      ...
      setStatus('Ready');
    } catch(e){ alert(e.message || 'Save failed'); setStatus('Ready'); }
  };

  const discardPending = () => { setPending(null); setQrExtras({ grade:'', railType:'', spec:'', lengthM:'' }); setStatus('Ready'); };
  const exportToExcel = async () => {
    ...
    try {
      ...
      const a = document.createElement('a'); a.href = href; a.download = `Master_${Date.now()}.xlsm`; a.click();
        ...
        <>
          ...
          <section className="card">
            ...
            <div className="grid" style={{gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
              ...
              <div>
                <label>Grade</label><input className="input" value={qrExtras.grade} readOnly />
              </div>
              <div>
                <label>Rail Type</label><input className="input" value={qrExtras.railType} readOnly />
              </div>
              <div>
                <label>Spec</label><input className="input" value={qrExtras.spec} readOnly />
              </div>
              <div>
                <label>Length (m)</label><input className="input" value={qrExtras.lengthM} readOnly />
              </div>
