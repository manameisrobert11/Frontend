...
function parseQrPayload(raw) {
  ...
  const grade = tokens.find(t => /^SAR\d{2}$/.test(t)) || '';
...
export default function App() {
  ...
  // operator + wagon IDs (three fields all labeled "Wagon ID")
  ...
  // NEW: controls-panel timing fields
  const [receivedAt, setReceivedAt] = useState(''); // "Recieved at" (spelling per request)
  ...
  // Remove scan action (confirmation)
  const handleRemoveScan = (scanId) => {
    setRemovePrompt(scanId);
  };
  ...
  const confirmPending = async () => {
    ...
    // Build record with THREE wagon IDs (all labeled "Wagon ID")
    // and the two new timing fields from the controls panel.
    const rec = {
      serial: pending.serial,
      stage: 'received',
      operator,
      // Keep new fields separate; not using old single "wagonId".
      wagonId1: wagonId1 || '',
      wagonId2: wagonId2 || '',
      wagonId3: wagonId3 || '',
      receivedAt: receivedAt || '', // "Recieved at"
      loadedAt: loadedAt || '',
    ...
    try {
      ...
      // reset only the parts we should reset
      setPending(null);
      setQrExtras({ grade: '', railType: '', spec: '', lengthM: '' });
  ...
  return (
    <div className="container" style={{ paddingTop: 20, paddingBottom: 20 }}>
      {/* Confirmation for removing scan */}
      {removePrompt && (
        <div
          role="dialog"
          aria-modal="true"
      ...
      <div className="grid" style={{ marginTop: 20 }}>
        ...
        <section className="card">
          ...
          <div className="controls-grid" style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
            ...
            {/* Three identical "Wagon ID" inputs (no 'Load ID', no 'Wagon 1/2/3' labels) */}
            <div>
          ...
          <div className="list">
            {scans.map((s) => (
              <div key={s.id} className="row">
                ...
                {/* Show three Wagon IDs if present */}
                ...
                {/* Show the new timing fields if present */}
                {(s.receivedAt || s.loadedAt) && (
                  <div className="meta">
                    {s.receivedAt ? `Recieved at: ${new Date(s.receivedAt).toLocaleString()}` : ''}
                    {s.receivedAt && s.loadedAt ? ' • ' : ''}
                    {s.loadedAt ? `Loaded at: ${new Date(s.loadedAt).toLocaleString()}` : ''}
                  </div>
                )}
