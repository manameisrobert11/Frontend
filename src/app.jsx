...
export default function App() {
  ...
  const handleRemoveScan = id=>{
    if(window.confirm('Remove this scan?')){
      fetch(api(`/staged/${id}`), { method:'DELETE' })
        .then(r=>r.json())
        .then(_=>setScans(prev=>prev.filter(s=>s.id!==id)))
        .catch(e=>alert(e.message||'Failed to remove scan'));
    }
  };
